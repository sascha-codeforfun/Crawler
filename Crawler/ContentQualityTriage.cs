...
namespace Crawler
{
	...
	public static class ContentQualityTriage
	{
			...
			string defaultLanguage = "")
		{
			...
			// ExcludeUrl: the same map also carries each UNWANTED_PATTERN_URL summary's
			// url occurrences, so a reviewed url-summary lists every reference too. Keys
			// are disjoint (distinct Word shapes), so merging cannot collide.
			var occurrenceContextsByKey =
				new Dictionary<string, List<string>>(WordCollisionContextsByKey(groups), StringComparer.OrdinalIgnoreCase);
			foreach (var kv in UrlOccurrencesByKey(groups))
			{
				occurrenceContextsByKey[kv.Key] = kv.Value;
			}

			ReviewTriagedQualityItems(issueTrackingPath, liveKeys, urlHighlightRules, occurrenceContextsByKey, config);
			...
			foreach (var group in groups)
			{
				...
				foreach (var line in group.DisplayLines)
				{
					...
					else if (group.DisplayType == "UNWANTED_PATTERN_URL"
						&& line.StartsWith("Loc     : ", StringComparison.Ordinal)
						&& !string.IsNullOrEmpty(group.Word))
					{
						// One "Loc     : [locator] url" line per url occurrence on the page.
						// Highlight the pattern within each so the operator sees the match in
						// every listed reference. The Word is "UNWANTED_PATTERN_URL:Category:
						// Name — pattern: X"; ExtractHighlightPatterns pulls X for the paint.
						var highlightPatterns = ExtractHighlightPatterns(group.Word);
						var loc = line["Loc     : ".Length..];
						ConsoleUi.WriteFieldInline("Loc");
						ConsoleUi.WriteWithPatternHighlight(loc, highlightPatterns);
						ConsoleUi.WriteBlank();
					}
		...
		// ── ExcludeUrl: per-page url-summary occurrence source ────────────────
		// Parallel to WordCollisionContextsByKey: a UNWANTED_PATTERN_URL summary
		// carries every url occurrence on the page as a "Loc     : " DisplayLine.
		// Keyed by the summary's tracking Key (= the promoted IssueRecord.Key), so
		// review and the per-page ticket list all occurrences from this run's log-10
		// accumulation rather than the collapsed record's single excerpt.

		/// <summary>
		/// Extracts each UNWANTED_PATTERN_URL summary's ordered occurrence lines (the
		/// text behind each "Loc     : " display line — "[locator] url"), keyed by the
		/// group's tracking key (= the promoted IssueRecord.Key).
		/// </summary>
		internal static Dictionary<string, List<string>> UrlOccurrencesByKey(
			IEnumerable<TriageGroup> groups)
		{
			const string prefix = "Loc     : ";
			var map = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
			foreach (var g in groups.Where(g => g.DisplayType == "UNWANTED_PATTERN_URL"))
			{
				var occurrences = g.DisplayLines
					.Where(l => l.StartsWith(prefix, StringComparison.Ordinal))
					.Select(l => l[prefix.Length..])
					.ToList();
				foreach (var key in g.TrackingKeys("new"))
		...
		/// <summary>
		/// Convenience overload for the ticket step (holds only the log path):
		/// re-accumulates this run's groups from log 10, then projects the
		/// UNWANTED_PATTERN_URL occurrences. Empty map when the log is absent.
		/// </summary>
		internal static Dictionary<string, List<string>> UrlOccurrencesByKey(
			string qualityLogPath, string qualityLogFilename,
			ContentQualityConfig config, string fileDownloadDirectory)
			...
			string defaultLanguage = "")
		{
			...
			//
			// EXCLUDEURL: occurrences the detector tagged as url-type (Detail carries
			// UnwantedPatterns.UrlMarker with a locator and the url token) are NOT
			// separate findings — they collapse into ONE summary per (page, pattern-name),
			// because every url occurrence shares the same fix (rename/redirect the path).
			// The summary still LISTS every occurrence (one DisplayLine each, with its
			// locator) so nothing is missed and the editor's ticket names them all. The
			// page's own-url culprit folds in as a "[page-url]" occurrence — the lone one
			// if nothing else survives. Untagged occurrences (visible text, comments,
			// slash-less refs) stay one finding each — each is a separate edit.
			var unwanted = lines.Where(x => x.IssueType == "UNWANTED_PATTERN").ToList();

			bool IsUrlTagged(string detail) =>
				detail.Contains(UnwantedPatterns.UrlMarker, StringComparison.Ordinal);

			foreach (var entry in unwanted.Where(e => !IsUrlTagged(e.Detail)))
			{
			...
			// Url-tagged occurrences → one summary per (page url, pattern prose). The
			// pattern prose is the Detail LEFT of the marker (identical for every
			// occurrence of one pattern); the payload right of the marker ("[loc] token")
			// is the per-occurrence locator+url that gets listed. Grouping by (url, prose)
			// keeps distinct patterns on a page as distinct summaries while collapsing the
			// repeated occurrences of one pattern.
			var urlTagged = unwanted.Where(e => IsUrlTagged(e.Detail))
				.Select(e =>
				{
					var idx = e.Detail.IndexOf(UnwantedPatterns.UrlMarker, StringComparison.Ordinal);
					var prose = e.Detail[..idx];
					var payload = e.Detail[(idx + UnwantedPatterns.UrlMarker.Length)..];
					return (Entry: e, Prose: prose, Payload: payload);
				})
				.GroupBy(x => (Url: ResolveUrl(x.Entry.Filename) is { Length: > 0 } u ? u : x.Entry.Filename,
					x.Prose))
				.OrderBy(g => g.Key.Url);

			foreach (var pageGroup in urlTagged)
			{
				var url = pageGroup.Key.Url;
				var prose = pageGroup.Key.Prose;
				// Occurrence lines in document order (log order), one per url occurrence.
				var occurrences = pageGroup.Select(x => x.Payload).ToList();
				var display = new List<string> { $"Pattern : {prose}" };
				display.AddRange(occurrences.Select(o => $"Loc     : {o}"));

				groups.Add(new TriageGroup(
					DisplayType: "UNWANTED_PATTERN_URL",
					Url: url,
					// One stable ledger row per (page, pattern prose): the Word keys on the
					// prose, NOT the per-occurrence payload, so the summary persists while
					// individual url occurrences come and go, and re-presents until all are
					// gone. Distinct from the per-occurrence Word shape so a url summary and
					// a stray non-url occurrence of the same pattern never collide.
					Word: $"UNWANTED_PATTERN_URL:{prose}",
					Comment: $"See {qualityLogFilename} — pattern '{prose}' in {occurrences.Count} url(s)",
					Excerpt: pageGroup.First().Entry.Context,
					IsTranslation: false,
			...
			// the title and each meta surface separately.
			//
			// IDENTITY: the detector packs a stable identity payload into Detail
			// after ControlChars.IdentitySeparator — a location token plus the FULL
			// marker-encoded element text. The Word (and thus the tracking Key
			// QUALITY|url|Word) is built from that payload, NOT from the display
			// prose. This is the fix for the previous instability: the old Word was
			// "CONTROL_CHARS_IN_CONTENT:Found <name> in <li> text", which is IDENTICAL
			// for every <li> on a page — so N distinct list items collapsed to one
			// key (Merge's GroupBy(Key).First() kept one, dropped the rest) and the
			// ticket never reached TicketText for the others. Keying on location +
			// encoded element text gives each distinct element its own stable key,
			// so all N surface and each survives an identical re-crawl.
			//
			// The display uses only the human prose (left of the separator); the
			// identity payload is machinery and is not shown.
			...
			foreach (var entry in controlChars)
			{
				...
				// Split display prose from the stable identity payload. Older logs
				// (written before the identity payload existed) have no separator —
				// fall back to the whole Detail as before, so a re-triage of a
				// pre-upgrade log still works (degrades to the old behaviour rather
				// than throwing).
				var sepIndex = entry.Detail.IndexOf(
					ControlChars.IdentitySeparator, StringComparison.Ordinal);
				string displayDetail;
				string identityPayload;
				if (sepIndex >= 0)
				{
					displayDetail = entry.Detail[..sepIndex];
					identityPayload = entry.Detail[(sepIndex + ControlChars.IdentitySeparator.Length)..];
				}
				else
				{
					displayDetail = entry.Detail;
					identityPayload = entry.Detail;
				}

				groups.Add(new TriageGroup(
					DisplayType: "CONTROL_CHARS_IN_CONTENT",
					Url: url,
					Word: $"CONTROL_CHARS_IN_CONTENT:{identityPayload}",
					Comment: $"See {qualityLogFilename} — {displayDetail}",
					Excerpt: entry.Context,
					IsTranslation: false,
					DisplayLines:
					[
			...
			ContentQualityConfig config)
		{
			...
			foreach (var (record, index) in reviewable)
			{
				...
				if (!string.IsNullOrEmpty(record.Excerpt))
				{
					...
					else if (displayType == "UNWANTED_PATTERN_URL")
					{
						// List every url occurrence on the page (one "Loc" line each), fed
						// from this run's accumulation keyed by record.Key — the same source
						// the ticket uses — so review shows the full reference list, not the
						// single stored excerpt. Falls back to the stored excerpt when no
						// live occurrences exist this run.
						var patterns = ExtractHighlightPatterns(record.Word);
						if (collisionContextsByKey.TryGetValue(record.Key, out var occ)
							&& occ.Count > 0)
						{
							foreach (var loc in occ)
							{
								ConsoleUi.WriteFieldInline("Loc");
								ConsoleUi.WriteWithPatternHighlightMuted(loc, patterns);
								ConsoleUi.WriteBlank();
							}
						}
						else
						{
							ConsoleUi.WriteInline(ctxPrefix);
							ConsoleUi.WriteWithPatternHighlightMuted(record.Excerpt, patterns);
							ConsoleUi.WriteBlank();
						}
					}
