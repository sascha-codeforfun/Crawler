	...
	internal static class AnalysisPipeline
	{
			...
			CrawlerRunContext ctx, Config config)
		{
			if (config.TicketGeneration.IsConfigured)
			{
				...
				// ExcludeUrl: a UNWANTED_PATTERN_URL summary row likewise expands to one
				// "Context:" line per url occurrence on the page, so the editor's ticket
				// lists every reference to fix. Both maps key by IssueRecord.Key and feed
				// the same Contexts mechanism, so they merge into one lookup — the ticket
				// renderer routes any row whose Key is present through the multi-Context
				// path regardless of which map supplied it. Keys are disjoint (distinct
				// Word shapes), so the merge cannot collide.
				var occurrenceContextsByKey =
					new Dictionary<string, List<string>>(collisionContextsByKey, StringComparer.OrdinalIgnoreCase);
				foreach (var kv in ContentQualityTriage.UrlOccurrencesByKey(
					ctx.ContentQualityLogPath, LogFileNames.ContentQualityIssues,
					config.ContentQuality, ctx.FileDownloadDirectory))
				{
