		...
		public static List<IssueRecord> PromoteFromQuality(string logPath)
		{
			...
			return File.ReadAllLines(logPath, Encoding.UTF8)
				...
				.Select(line =>
				{
					...
					// the Detail field (e.g. "UNWANTED_PATTERN:Legacy-Product: Old
					// Product — pattern: kwitt", or "MALFORMED_HTML:CONTENT_BEFORE_DOCTYPE").
					...
					// Key space.
					var word = issueType is "UNWANTED_PATTERN" or "MALFORMED_HTML"
							? (string.IsNullOrEmpty(detail) ? issueType : $"{issueType}:{detail}")
							: issueType;
