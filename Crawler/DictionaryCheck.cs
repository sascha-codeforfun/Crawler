namespace Crawler
{
	...
	{
		public record DictionaryAnalysis(
		...
		/// </summary>
		public static (DictionaryAnalysis User, DictionaryAnalysis Site) AnalyseDictionaries(
			string userDictionaryPath,
			string siteDictionaryPath,
			string folderPath,
			string outputFilePath,
			IReadOnlyList<string>? prefixesToStrip = null,
			IReadOnlyCollection<DictionaryBundle>? loadedBundles = null,
		...
		/// </summary>
		internal static DictionaryAnalysis AnalyseDictionary(
			string dictionaryPath,
			string folderPath,
			IReadOnlyList<string>? prefixesToStrip,
			IReadOnlyCollection<DictionaryBundle>? loadedBundles,
			int degreeOfParallelism = 0)
		{
			...
			// The ! is stripped when loading into DictionaryBundle so spell-check is unaffected.
			...
			{
				return new DictionaryAnalysis(orphaned, redundant);
			}
			...
			// DictionaryBundle.CheckAny is not thread-safe.
			if (prefixesToStrip is { Count: > 0 } && loadedBundles is { Count: > 0 })
			{
				foreach (var word in analysisWords)
				{
					...
					var remainder = Tools.StripWordPrefixPublic(word, prefixesToStrip);
					if (remainder == null || remainder == word)
					{
						continue;
					}

					if (DictionaryBundle.CheckAny(remainder, loadedBundles))
					{
						redundant.Add(word);
					}
				}
		...
		/// during spell-check (collected by <see cref="DictionaryUsageTracker"/>).
		...
		/// dictionary entries.
		/// </summary>
		public static DictionaryAnalysis AnalyseFromUsage(
			string dictionaryPath,
		...
		private static void WriteAnalysisLog(
			string outputFilePath,
			string userPath,
			DictionaryAnalysis user,
			string sitePath,
			DictionaryAnalysis site,
