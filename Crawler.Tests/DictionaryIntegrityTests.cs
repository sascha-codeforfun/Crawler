...
namespace Crawler.Tests
{
	...
	/// </summary>
	[Collection("Logger")]
	public class DictionaryIntegrityTests : IDisposable
	{
		private readonly string _tempDir;

		public DictionaryIntegrityTests()
		{
		...
		public void ComputeSha256_KnownContent_ReturnsExpectedHash()
		{
			...
			var actual = DictionaryIntegrity.ComputeSha256(path);
		...
		public void ComputeSha256_DifferentContent_DifferentHash()
		{
			...
			Assert.NotEqual(
				DictionaryIntegrity.ComputeSha256(path1),
				DictionaryIntegrity.ComputeSha256(path2));
		}
		...
		public void VerifyField_MatchingChecksum_ReturnsPass()
		{
			...
			var result = DictionaryIntegrity.VerifyField(
				"de", "DicFile", path, "DicChecksum", sum);

			Assert.Equal(DictionaryIntegrity.FieldStatus.Pass, result.Status);
		...
		public void VerifyField_EmptyChecksum_ReturnsMissingChecksum()
		{
			...
			Assert.Equal(DictionaryIntegrity.FieldStatus.MissingChecksum, result.Status);
			Assert.Equal(sum, result.ActualChecksum);
		...
		public void VerifyField_MismatchedChecksum_ReturnsMismatch()
		{
			...
			Assert.Equal(DictionaryIntegrity.FieldStatus.Mismatch, result.Status);
		...
		public void VerifyField_MissingFile_ReturnsMissingFile()
		{
			...
			Assert.Equal(DictionaryIntegrity.FieldStatus.MissingFile, result.Status);
		...
		{
			var result = DictionaryIntegrity.VerifyAll([]);
			Assert.Empty(result);
		}
		...
		public void VerifyAll_AllPass_ReturnsAllPass()
		{
			...
			Assert.All(result, r => Assert.Equal(DictionaryIntegrity.FieldStatus.Pass, r.Status));
		}
		...
		public void VerifyAll_MixedFailures_AllReported()
		{
			...
			Assert.Equal(2, result.Count(r => r.Status == DictionaryIntegrity.FieldStatus.Pass
				&& r.LanguageCode == "ok"));
			Assert.Equal(2, result.Count(r => r.Status == DictionaryIntegrity.FieldStatus.MissingChecksum
				&& r.LanguageCode == "new"));
			Assert.Equal(1, result.Count(r => r.Status == DictionaryIntegrity.FieldStatus.Mismatch
				&& r.LanguageCode == "bad"));
		}
		...
		{
			List<DictionaryIntegrity.FieldResult> failures =
			[
				new("de", "DicFile", "dictionaries/de.dic", "DicChecksum",
					DictionaryIntegrity.FieldStatus.MissingChecksum,
					ExpectedChecksum: "",
					ActualChecksum: new string('1', 64)),
			];

			var msg = DictionaryIntegrity.BuildHaltMessage(failures);
		...
		public void BuildHaltMessage_OnlyMismatch_ContainsMismatchSection()
		{
			...
			[
				new("de", "DicFile", "dictionaries/de.dic", "DicChecksum",
					DictionaryIntegrity.FieldStatus.Mismatch,
		...
		public void BuildHaltMessage_OnlyMissingFile_ContainsMissingFileSection()
		{
			...
			[
				new("de", "DicFile", "dictionaries/missing.dic", "DicChecksum",
					DictionaryIntegrity.FieldStatus.MissingFile,
					ExpectedChecksum: "",
					ActualChecksum:   ""),
			];
		...
		public void CheckOrHalt_NullConfig_ReturnsTrue()
		{
			...
			// don't crash).
			Assert.True(DictionaryIntegrity.CheckOrHalt(null!));
		}
		...
		public void CheckOrHalt_NoBundles_ReturnsTrue()
		{
			...
			Assert.True(DictionaryIntegrity.CheckOrHalt(config));
		}
		...
		public void CheckOrHalt_BundleMismatch_ReturnsFalse()
		{
			...
			Assert.False(DictionaryIntegrity.CheckOrHalt(config));
		}
	}
}
