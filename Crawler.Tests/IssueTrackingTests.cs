...
namespace Crawler.Tests
{
	...
	public class IssueTrackingTests : IDisposable
	{
		...
		// ── PromoteFromQuality: CONTROL_CHARS identity round-trip ─────────────
		// A control-char finding's Detail carries a stable identity payload after
		// the ControlChars identity separator (a location token plus the full
		// marker-encoded element text). BOTH consumers of the content-quality log
		// must key the Word on that payload: the triage builder (ContentQualityTriage)
		// and this auto-promote parser (PromoteFromQuality, which feeds Merge's
		// detected set). If they diverge, a ticketed pending row and the detected
		// record for the SAME finding get different Keys, so Merge's gone-is-gone
		// drops the ticket and the finding re-presents every run. These tests lock
		// the two sides to the same Word so a future format change to one side
		// fails here instead of silently dropping adjudicated decisions.
		//
		// Fixtures are synthetic: invented element text, example filenames, the
		// real marker/separator constants (which are a frozen stability contract).

		private const string Sep = Crawler.Quality.ControlChars.IdentitySeparator;

		// The Word the triage builder produces for a given content-quality Detail.
		// Mirrors ContentQualityTriage's control-chars group exactly (split on the
		// identity separator, prefix with the type). Kept here so the equality test
		// asserts PromoteFromQuality matches the triage rule without standing up the
		// whole triage UI.
		private static string TriageWord(string detail)
		{
			var i = detail.IndexOf(Sep, StringComparison.Ordinal);
			var payload = i >= 0 ? detail[(i + Sep.Length)..] : detail;
			return "CONTROL_CHARS_IN_CONTENT:" + payload;
		}

		private static string CcDetail(string prose, string location, string encodedText)
			=> prose + Sep + location + " :: " + encodedText;

		[Fact]
		public void PromoteFromQuality_ControlChars_WordKeysOnIdentityPayload_NotBareType()
		{
			var detail = CcDetail(
				"Found ZWSP (U+200B) in <li> text",
				"<li>",
				"item one[INVISIBLE ZERO-WIDTH SPACE U+200B]");
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				$"page.html|CONTROL_CHARS_IN_CONTENT|{detail}|item one[INVISIBLE ZERO-WIDTH SPACE U+200B]");

			var result = IssueTracking.PromoteFromQuality(path);

			var record = Assert.Single(result);
			// Word must carry the identity payload, NOT the bare type.
			Assert.Equal("CONTROL_CHARS_IN_CONTENT:<li> :: item one[INVISIBLE ZERO-WIDTH SPACE U+200B]", record.Word);
			Assert.NotEqual("CONTROL_CHARS_IN_CONTENT", record.Word);
		}

		[Fact]
		public void PromoteFromQuality_ControlChars_WordEqualsTriageWord()
		{
			// The invariant that broke: the detected (auto-promote) Word and the
			// triage Word for the same finding must be identical.
			var detail = CcDetail(
				"Found LF (U+000A) in meta[@name=\"description\"] content",
				"meta[name=description]",
				"some description text[LF]");
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				$"page.html|CONTROL_CHARS_IN_CONTENT|{detail}|some description text[LF]");

			var record = Assert.Single(IssueTracking.PromoteFromQuality(path));
			Assert.Equal(TriageWord(detail), record.Word);
		}

		[Fact]
		public void PromoteFromQuality_ControlChars_FourListItems_FourDistinctWords()
		{
			// Four <li> on one page must yield four distinct Words (not collapse to
			// one via DistinctBy(Key)). This is the original four-into-one bug seen
			// from the auto-promote side.
			string Line(string text) =>
				"page.html|CONTROL_CHARS_IN_CONTENT|" +
				CcDetail("Found ZWSP (U+200B) in <li> text", "<li>",
					$"{text}[INVISIBLE ZERO-WIDTH SPACE U+200B]") +
				$"|{text}[INVISIBLE ZERO-WIDTH SPACE U+200B]";

			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				Line("item one"), Line("item two"), Line("item three"), Line("item four"));

			var result = IssueTracking.PromoteFromQuality(path);

			Assert.Equal(4, result.Count);
			Assert.Equal(4, result.Select(r => r.Word).Distinct().Count());
			Assert.Equal(4, result.Select(r => r.Key).Distinct().Count());
		}

		[Fact]
		public void PromoteFromQuality_ControlChars_NoSeparator_FallsBackToWholeDetail()
		{
			// A pre-upgrade log line without the identity separator falls back to the
			// whole Detail as payload — matching the triage fallback, so old logs still
			// reconcile rather than throwing.
			var detail = "Found ZWSP (U+200B) in <li> text";
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				$"page.html|CONTROL_CHARS_IN_CONTENT|{detail}|ctx");

			var record = Assert.Single(IssueTracking.PromoteFromQuality(path));
			Assert.Equal($"CONTROL_CHARS_IN_CONTENT:{detail}", record.Word);
			Assert.Equal(TriageWord(detail), record.Word);
		}

		[Fact]
		public void PromoteFromQuality_ControlChars_TicketedRowSurvivesMerge()
		{
			// End-to-end: the detected record (from PromoteFromQuality) and a ticketed
			// pending record (built with the triage Word) share a Key, so Merge keeps
			// the pending row. This is the round-trip whose failure caused ticketed
			// control-chars to be dropped and re-presented every run.
			var detail = CcDetail(
				"Found ZWSP (U+200B) in <li> text",
				"<li>",
				"keep me[INVISIBLE ZERO-WIDTH SPACE U+200B]");
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				$"page.html|CONTROL_CHARS_IN_CONTENT|{detail}|keep me[INVISIBLE ZERO-WIDTH SPACE U+200B]");

			var detected = IssueTracking.PromoteFromQuality(path);
			var detectedRecord = Assert.Single(detected);

			// A ticketed pending row keyed with the triage Word for the same finding.
			var ticketed = New("QUALITY", detectedRecord.Url, TriageWord(detail), status: "pending");

			var merged = IssueTracking.Merge(
				new List<IssueTracking.IssueRecord> { ticketed },
				detected);

			var survivor = merged.SingleOrDefault(r =>
				r.Url == detectedRecord.Url && r.Word == TriageWord(detail));
			Assert.NotNull(survivor);
			Assert.Equal("pending", survivor!.Status);
		}

		// ── PromoteFromQuality: ExcludeUrl url-summary round-trip ─────────────
		// A url-tagged UNWANTED_PATTERN occurrence (Detail carries the ExcludeUrl
		// marker) keys as a UNWANTED_PATTERN_URL summary on the pattern prose left of
		// the marker, so every url occurrence on a page collapses to one Key and a
		// ticketed summary round-trips. Untagged occurrences keep the per-occurrence
		// composite Word. Synthetic fixtures.

		private const string UrlMark = Crawler.Quality.UnwantedPatterns.UrlMarker;

		// The summary Word the triage builder produces for a url-tagged Detail:
		// UNWANTED_PATTERN_URL:{prose left of the marker}.
		private static string UrlSummaryWord(string detail)
		{
			var i = detail.IndexOf(UrlMark, StringComparison.Ordinal);
			var prose = i >= 0 ? detail[..i] : detail;
			return "UNWANTED_PATTERN_URL:" + prose;
		}

		[Fact]
		public void PromoteFromQuality_Url_TaggedOccurrence_KeysAsUrlSummary()
		{
			var prose = "Legacy: Old — pattern: oldword";
			var detail = $"{prose}{UrlMark}[src] /a/oldword.jpg";
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				$"page.html|UNWANTED_PATTERN|{detail}|ctx");

			var record = Assert.Single(IssueTracking.PromoteFromQuality(path));
			Assert.Equal($"UNWANTED_PATTERN_URL:{prose}", record.Word);
			Assert.Equal(UrlSummaryWord(detail), record.Word);
		}

		[Fact]
		public void PromoteFromQuality_Url_Untagged_KeepsPerOccurrenceWord()
		{
			var detail = "Legacy: Old — pattern: oldword";
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				$"page.html|UNWANTED_PATTERN|{detail}|ctx");

			var record = Assert.Single(IssueTracking.PromoteFromQuality(path));
			Assert.Equal($"UNWANTED_PATTERN:{detail}", record.Word);
		}

		[Fact]
		public void PromoteFromQuality_Url_ManyOccurrencesOnePage_CollapseToOneKey()
		{
			var prose = "Legacy: Old — pattern: oldword";
			string Line(string loc, string urlv) =>
				$"page.html|UNWANTED_PATTERN|{prose}{UrlMark}[{loc}] {urlv}|ctx";

			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				Line("src", "/a/oldword.jpg"),
				Line("src", "/b/oldword.jpg"),
				Line("link", "/c/oldword.html"),
				Line("page-url", "https://example.com/oldword.html"));

			var result = IssueTracking.PromoteFromQuality(path);

			// DistinctBy(Key) collapses the four url occurrences to one summary record.
			var record = Assert.Single(result);
			Assert.Equal($"UNWANTED_PATTERN_URL:{prose}", record.Word);
		}

		[Fact]
		public void PromoteFromQuality_Url_SummaryRowSurvivesMerge()
		{
			// The detected summary (from PromoteFromQuality) and a ticketed pending
			// summary (triage Word) share a Key, so Merge keeps the ticket — the round
			// trip that keeps a decided url summary from being dropped and re-presented.
			var prose = "Legacy: Old — pattern: oldword";
			var detail = $"{prose}{UrlMark}[src] /a/oldword.jpg";
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
			...
			var ticketed = New("QUALITY", detectedRecord.Url, UrlSummaryWord(detail), status: "pending");
			...
			var survivor = merged.SingleOrDefault(r =>
				r.Url == detectedRecord.Url && r.Word == UrlSummaryWord(detail));
			Assert.NotNull(survivor);
			Assert.Equal("pending", survivor!.Status);
		}
		...
		public void PromoteFromQuality_UnwantedPattern_WordIsComposite()
		{
			...
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				"page.html|UNWANTED_PATTERN|Legacy-Product: Old Product — pattern: oldword|...content with oldword...");
			var result = IssueTracking.PromoteFromQuality(path);

			Assert.Single(result);
			Assert.Equal("UNWANTED_PATTERN:Legacy-Product: Old Product — pattern: oldword",
				result[0].Word);
			// SourceLabel still carries the detail (unchanged behaviour).
			Assert.Equal("Legacy-Product: Old Product — pattern: oldword",
		...
		public void PromoteFromQuality_UnwantedPatternCompositeKey_MatchesTriageKey()
		{
			...
			var path = TmpFile("quality.log",
				"Filename|IssueType|Detail|Context",
				"page.html|UNWANTED_PATTERN|Legacy-Product: Old Product — pattern: oldword|content");
			...
			var triageDecision = new IssueTracking.IssueRecord
			{
				...
				Word = "UNWANTED_PATTERN:Legacy-Product: Old Product — pattern: oldword",
				Status = "wontfix",
			};
