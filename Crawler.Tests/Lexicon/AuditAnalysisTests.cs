...
{
	/// <summary>
	/// Synthetic-fixture tests for Audit.AnalyseDictionary. These tests
	...
	/// The synthetic system dictionary is constructed via Bundle's
	/// SharedSite/SharedUser HashSets — the WordList-backed System field is left
	/// null since Bundle.Check short-circuits to the shared sets first.
	/// </summary>
	[Collection("Logger")]
	public class AuditAnalysisTests : IDisposable
	{
		private readonly string _tempDir;
		private readonly string _corpusDir;

		public AuditAnalysisTests()
		{
		...
		{
			// Use SharedSite — Bundle.Check returns true on SharedUser/SharedSite
		...
		{
			// Hunspell-style affix flags: 'word/AB' — Audit filters these out
		...
		public void AnalyseDictionary_RespectsExplicitDegreeOfParallelism()
		{
			...
			var serial = Audit.AnalyseDictionary(
				dict, _corpusDir,
				prefixesToStrip: null, loadedBundles: null,
				degreeOfParallelism: 1);

			var parallel = Audit.AnalyseDictionary(
				dict, _corpusDir,
				prefixesToStrip: null, loadedBundles: null,
