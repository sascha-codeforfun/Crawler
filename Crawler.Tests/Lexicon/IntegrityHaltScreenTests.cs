...
namespace Crawler.Tests.Lexicon
{
	...
	/// </summary>
	public class IntegrityHaltScreenTests
	{
		private static Integrity.FieldResult R(
			string lang, string field, Integrity.FieldStatus status,
		...
		{
			var blocks = Integrity.BuildHaltBlocks(
				new List<DictionaryBundleConfig> { B("de", "German") },
				new List<Integrity.FieldResult>
				{
					R("de", "DicFile", Integrity.FieldStatus.Mismatch, "OLDHASH", "NEWHASH"),
					R("de", "AffFile", Integrity.FieldStatus.Pass),
				});
		...
		public void MissingChecksum_ConfiguredIsAmber_NotRed()
		{
			var blocks = Integrity.BuildHaltBlocks(
				...
				new List<Integrity.FieldResult>
				{
					R("en", "DicFile", Integrity.FieldStatus.Pass),
					R("en", "AffFile", Integrity.FieldStatus.MissingChecksum, "", "FRESHHASH"),
				});
		...
		public void MissingFile_IsRed_WithPath_AndLeadsTheBundle()
		{
			var blocks = Integrity.BuildHaltBlocks(
				...
				new List<Integrity.FieldResult>
				{
					R("pt", "DicFile", Integrity.FieldStatus.MissingFile, path: "dictionaries/pt_PT.dicNope"),
					R("pt", "AffFile", Integrity.FieldStatus.Pass),
				});
		...
		public void MissingDisplayName_HaltsTheBundle_HeaderHasNoName_NameLineIsAmber()
		{
			...
			var blocks = Integrity.BuildHaltBlocks(
				...
				new List<Integrity.FieldResult>
				{
					R("sk", "DicFile", Integrity.FieldStatus.Pass),
					R("sk", "AffFile", Integrity.FieldStatus.Pass),
				});
		...
		public void CleanNamedBundle_ProducesNoBundleBlock_AndOrderIsStable()
		{
			...
			var blocks = Integrity.BuildHaltBlocks(
				...
				new List<Integrity.FieldResult>
				{
					R("el", "DicFile", Integrity.FieldStatus.Pass),
					R("el", "AffFile", Integrity.FieldStatus.Pass),
					R("de", "DicFile", Integrity.FieldStatus.Mismatch, "OLD", "NEW"),
					R("de", "AffFile", Integrity.FieldStatus.Pass),
				});
