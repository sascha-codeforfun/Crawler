	...
	public class ContentQualityReviewTests
	{
		private static IssueTracking.IssueRecord Triaged(
			...
			=> new()
			{
				...
				Word = "UNWANTED_PATTERN:oldword",
				Comment = comment,
				Language = "de",
				SourceLabel = "span[#text]",
				Excerpt = "pay with oldword here",
			};
		...
		public void ExtractHighlightPatterns_SingularMarker_ReturnsOnePattern()
		{
			var result = ContentQualityTriage.ExtractHighlightPatterns(
				"UNWANTED_PATTERN:Legacy: Old Name — pattern: oldword");
			Assert.Equal(new[] { "oldword" }, result);
		}
		...
		{
			var result = ContentQualityTriage.ExtractHighlightPatterns(
				"UNWANTED_PATTERN:Legacy: Old Name — patterns: oldword, otherword");
			Assert.Equal(new[] { "oldword", "otherword" }, result);
		}

		[Fact]
		public void ExtractHighlightPatterns_NoMarker_ReturnsAfterColon()
		{
			var result = ContentQualityTriage.ExtractHighlightPatterns("UNWANTED_PATTERN:oldword");
			Assert.Equal(new[] { "oldword" }, result);
		}
