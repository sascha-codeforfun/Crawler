...
{
	/// <summary>
	/// Unit tests for CrawlLog.Analyse — the pure log-analysis helper
	...
	///    Analyse determines snapshot health by checking only the
	...
	/// </summary>
	public class CrawlLogTests : IDisposable
	{
		private readonly string _tempDir;

		public CrawlLogTests()
		{
		...
		[Fact]
		public void Analyse_MissingFile_ReturnsIncomplete()
		{
			var (isComplete, duration, lastEntry) = CrawlLog.Analyse(
		...
		[Fact]
		public void Analyse_EmptyFile_ReturnsIncomplete()
		{
			var path = WriteLog();
			var (isComplete, duration, lastEntry) = CrawlLog.Analyse(
		...
		[Fact]
		public void Analyse_OnlyBlankLines_ReturnsIncomplete()
		{
			var path = WriteLog("", "  ", "\t", "");
			var (isComplete, _, lastEntry) = CrawlLog.Analyse(
				path, "https://example.com");
			Assert.False(isComplete);
			Assert.Equal("", lastEntry);
		}
		...
		[Fact]
		public void Analyse_LastLineCompleted_ReturnsComplete()
		{
			var path = WriteLog(
		...
		[Fact]
		public void Analyse_LastLineCompletedFollowedByBlank_StillComplete()
		{
			...
			var path = WriteLog(
				...
				"",
				"   ");
			var (isComplete, _, _) = CrawlLog.Analyse(
				path, "https://example.com");
			Assert.True(isComplete);
		}
		...
		[Fact]
		public void Analyse_TwoCompletedMarkers_LastIsPostCrawl_Complete()
		{
		...
		[Fact]
		public void Analyse_CompletedFollowedByMoreEntries_Incomplete()
		{
		...
		[Fact]
		public void Analyse_NoCompletedMarker_Incomplete()
		{
			var path = WriteLog(
				"2026-04-25 12:00:00 | https://example.com | 200 | crawled",
		...
		[Fact]
		public void Analyse_ParsesDurationFromFirstCrawledToLastCompleted()
		{
			var path = WriteLog(
				"2026-04-25 12:00:00 | https://example.com | 200 | crawled",
				"2026-04-25 12:00:15 | https://example.com/p1 | 200 | saved",
				"2026-04-25 12:00:30 | https://example.com | OK  | completed | info");
			var (isComplete, duration, _) = CrawlLog.Analyse(
				path, "https://example.com");
			Assert.True(isComplete);
		...
		[Fact]
		public void Analyse_DurationUsesSecondCompletedWithPostCrawlPass()
		{
			...
			var (_, duration, _) = CrawlLog.Analyse(
				path, "https://example.com");
			Assert.NotNull(duration);
		...
		[Fact]
		public void Analyse_NoMatchingCrawledLine_DurationNull()
		{
		...
		[Fact]
		public void Analyse_UnparseableTimestamp_DurationNull()
		{
			var path = WriteLog(
		...
		[Fact]
		public void ParseTimestamp_StandardFormat_Parses()
		{
			var result = CrawlLog.ParseTimestamp("2026-04-25 12:07:22 | url | status | message");
			Assert.NotNull(result);
			Assert.Equal(new DateTime(2026, 4, 25, 12, 7, 22), result);
		}

		[Fact]
		public void ParseTimestamp_NoPipe_TriesParseWholeLine()
		{
			var result = CrawlLog.ParseTimestamp("2026-04-25 12:07:22");
			Assert.NotNull(result);
		}

		[Fact]
		public void ParseTimestamp_GarbageInput_ReturnsNull()
		{
			var result = CrawlLog.ParseTimestamp("garbage | content | here");
			Assert.Null(result);
		}

		[Fact]
		public void ParseTimestamp_NonExactButParseableFormat_UsesLenientFallback()
		{
			// "2026-04-25T12:07:22" fails the exact "yyyy-MM-dd HH:mm:ss" pattern (T
			// separator) but DateTime.TryParse accepts ISO-8601 — exercises the
			// lenient fallback branch.
			var result = CrawlLog.ParseTimestamp("2026-04-25T12:07:22 | url | status");
			Assert.NotNull(result);
			Assert.Equal(new DateTime(2026, 4, 25, 12, 7, 22), result);
		}
	}
