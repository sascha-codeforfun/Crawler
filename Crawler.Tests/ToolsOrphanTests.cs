...
{
	/// <summary>
	/// Unit tests for Tools.DetectOrphans — the pure orphan-detection helper
	/// extracted from Program.CrawlerIndexIntegrityCheck in fileset #271.
	/// </summary>
	public class ToolsOrphanTests : IDisposable
	{
		private readonly string _tempDir;

		public ToolsOrphanTests()
		{
		...
		{
			// Index format: "filename | url | source"  — DetectOrphans only reads first column
		...
		[Fact]
		public void DetectOrphans_MissingDownloadDirectory_ReturnsEmpty()
		{
			var index = CreateIndex("a.html");
			var orphans = Tools.DetectOrphans(
				Path.Combine(_tempDir, "no-such-dir"), index, "*.html");
			Assert.Empty(orphans);
		}

		[Fact]
		public void DetectOrphans_MissingIndexFile_ReturnsEmpty()
		{
		...
		[Fact]
		public void DetectOrphans_EmptyDirectoryEmptyIndex_ReturnsEmpty()
		{
			var dir = CreateDownloadDir();
			var index = CreateIndex(); // empty
			var orphans = Tools.DetectOrphans(dir, index, "*.html");
			Assert.Empty(orphans);
		}
		...
		[Fact]
		public void DetectOrphans_AllFilesIndexed_ReturnsEmpty()
		{
			var dir = CreateDownloadDir();
			WriteFile(dir, "a.html");
		...
		[Fact]
		public void DetectOrphans_OneOrphanPresent_ReturnsIt()
		{
			var dir = CreateDownloadDir();
			WriteFile(dir, "a.html");
		...
		[Fact]
		public void DetectOrphans_MultipleOrphans_ReturnsAllSortedAscending()
		{
			var dir = CreateDownloadDir();
		...
		[Fact]
		public void DetectOrphans_IndexEntriesNotInDirectory_AreIgnored()
		{
			// An indexed-but-missing file is not an orphan — DetectOrphans only
		...
		[Fact]
		public void DetectOrphans_CaseInsensitiveFilenameMatch()
		{
			var dir = CreateDownloadDir();
		...
		[Fact]
		public void DetectOrphans_RespectsFilePattern_NonMatchingExtensionsIgnored()
		{
			var dir = CreateDownloadDir();
			WriteFile(dir, "a.html");
		...
		[Fact]
		public void DetectOrphans_IndexBlankLines_AreSkipped()
		{
			var dir = CreateDownloadDir();
			WriteFile(dir, "a.html");
			...
			var orphans = Tools.DetectOrphans(dir, indexPath, "*.html");
			Assert.Empty(orphans);
		}

		[Fact]
		public void DetectOrphans_IndexEntryFirstColumnTrimmed()
		{
