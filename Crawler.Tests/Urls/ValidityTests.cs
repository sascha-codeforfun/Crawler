...
{
	/// <summary>
	/// Locks the behavior of <see cref="Validity.IsInDownloadScope"/>, the security
	...
	public class ValidityTests
	{
		...
		{
			Assert.True(Validity.IsInDownloadScope(
				$"{Site}/page.html",
				Site,
				NoExclusions));
		}
		...
		public void AbsoluteForeignDomain_Rejected()
		{
			...
			// regressions.
			Assert.False(Validity.IsInDownloadScope(
		...
		public void AllowedSubdomain_WhitespaceEntryIgnored()
		{
			...
			// defensive check inside IsInDownloadScope). Verify by configuring only
		...
		public void PrimaryDomainRootUrl_Accepted()
		{
			...
			Assert.True(Validity.IsInDownloadScope(Site, Site, NoExclusions));
		}
		...
		//
		// IsInDownloadScope must treat empty/whitespace entries as non-matches so
