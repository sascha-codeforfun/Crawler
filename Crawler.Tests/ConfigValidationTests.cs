	...
	public class ConfigValidationTests : IDisposable
	{
			...
			var path = WriteJson("""
				{
					...
					"MaxConcurrentAssetDownloads": 1,
					"TicketGeneration": {
						"PathShortenSegments": [ "privatkunden", "pensions" ]
					}
				}
				""");
			var config = Config.LoadFromJson(path);
		...
		public void LoadFromJson_PathShortenSegmentsShortEntry_Silent_WarnsAndSkips()
		{
			...
			try
			{
				...
				var path = WriteJson("""
					{
						...
						"MaxConcurrentAssetDownloads": 1,
						"TicketGeneration": {
							"PathShortenSegments": [ "de", "abc", "privatkunden", "pensions" ]
						}
					}
					""");
				var config = Config.LoadFromJson(path);
				...
				Assert.Contains("pensions", config.TicketGeneration.PathShortenSegments);
