	...
	public class DictionaryCheckTests : IDisposable
	{
		...
		public void BackupDictionary_NonExistentFile_NoOp()
		{
			var ex = Record.Exception(() =>
				DictionaryCheck.BackupDictionary(Path.Combine(_tempDir, "missing.dic")));
			Assert.Null(ex);
		}
		...
		public void BackupDictionary_CreatesNumberedBackup()
		{
			...
			DictionaryCheck.BackupDictionary(dic);
		...
		{
			var ex = Record.Exception(() =>
				DictionaryCheck.SortDictionary(Path.Combine(_tempDir, "missing.dic")));
			Assert.Null(ex);
		}
		...
		public void SortDictionary_AlreadySorted_NoBackupCreated()
		{
			...
			DictionaryCheck.SortDictionary(dic);
		...
		{
			var ex = Record.Exception(() =>
				DictionaryCheck.CleanDictionary(
		...
		public void CleanDictionary_EmptyToRemove_NoBackupCreated()
		{
			...
			DictionaryCheck.CleanDictionary(dic, new Dictionary<string, string>());
			Assert.False(File.Exists(dic + ".1"));
		}
		...
		public void CleanDictionary_RemovesFlaggedEntry()
		{
			...
			DictionaryCheck.CleanDictionary(dic,
		...
		public void CleanDictionary_RemovesDuplicates()
		{
			...
			DictionaryCheck.CleanDictionary(dic2,
		...
		public void PinWords_EmptyList_NoBackupCreated()
		{
			...
			DictionaryCheck.PinWords(dic, Array.Empty<string>());
			Assert.False(File.Exists(dic + ".1"));
		}
		...
		{
			var dic = DicFile("pin.dic", "word1", "word2");
			DictionaryCheck.PinWords(dic, new[] { "word1" });
			var lines = File.ReadAllLines(dic);
			Assert.Contains("!word1", lines);
		}
		...
		{
			var dic = DicFile("pin.dic", "word1", "word2");
			DictionaryCheck.PinWords(dic, new[] { "nonexistent" });
			Assert.False(File.Exists(dic + ".1"));
		}
		...
		public void PinWords_MultipleWords_AllPinned()
		{
			...
			DictionaryCheck.PinWords(dic, new[] { "word1", "word3" });
			var lines = File.ReadAllLines(dic);
			Assert.Contains("!word1", lines);
			Assert.Contains("!word3", lines);
			Assert.Contains("word2", lines);
		...
		public void AnalyseDictionary_WordNotInCorpus_Orphaned()
		{
			...
			var result = DictionaryCheck.AnalyseDictionary(dic, _tempDir, null, null);
